 ...
 * 추출 결과는 도메인으로 들어오기 전에 schemas/career-evidence.llm.schema.json 으로
 ...
 * </pre>
 *
 * <p>배열이 아니라 객체로 감싸는 이유는 Structured Output 이 루트를 object 로 요구하기
 * 때문이다. 이 스키마 하나가 곧 Structured Output 계약이자 이 포트의 반환 계약이며
 * Validator 의 입력 계약이다.
 *
 * <p>이력서나 프로젝트 설명 하나에 성능 개선·장애 분석·협업처럼 독립적인 경험이 여러 개
 * 들어 있는 것이 정상이고, 그것을 하나로 뭉치면 Fit Analysis 에서 재사용할 수 없다.
